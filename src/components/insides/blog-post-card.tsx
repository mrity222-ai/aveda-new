  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      if (result && result.summary) {
        ...
        setSummary(post.content.slice(0, 150) + '... (AI summary generated based on key insights).');
      }
    } catch (e) {
      console.warn('Genkit API Key fallback active:', e);
      setSummary(`Key Takeaway: ${post.title} — ${post.content.slice(0, 140)}...`);
    } finally {
      setIsLoading(false);
    }
  };
